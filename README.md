...
This project investigates whether augmenting text-to-SQL systems with natural language semantic layers significantly improves query generation accuracy compared to schema-only approaches. We will build a complete cloud-native text-to-SQL system that generates rich, human-readable database documentation and evaluates its impact on SQL generation quality.

**Core Innovation:** Using LLM-generated natural language documentation (vs. structured ontologies) as a semantic layer that describes database meaning in business terms, including column purposes, synonyms, relationships, and common query patterns.

**Expected Outcome:** Demonstrate 15-25% accuracy improvement on Spider benchmark when semantic layer is included in the text-to-SQL pipeline.

**Deliverables:**
- Working cloud-deployed application with comparison mode
- Natural language semantic layer for 15-20 databases
- Comprehensive evaluation on Spider 1.0 benchmark
- Demo presentation
...
### Research Question
**"Does augmenting text-to-SQL systems with natural language semantic layers improve SQL generation accuracy compared to using database schemas alone?"**

### Hypothesis
Natural language semantic documentation that includes business context, column meanings, synonyms, relationship explanations, and query patterns will enable LLMs to generate more accurate SQL queries than raw database schemas (DDL) alone.

### Why This Matters
1. **Practical Problem:** LLMs struggle with ambiguous column names, business logic, and domain terminology
2. **Current Gap:** Most approaches use structured ontologies (complex, hard to maintain) or no semantic layer at all
3. **Our Approach:** Natural language documentation is human-readable, maintainable, and LLM-friendly
4. **Enterprise Value:** Documentation itself is valuable, making this approach dual-purpose
