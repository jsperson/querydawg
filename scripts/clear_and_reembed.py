#!/usr/bin/env python3
"""
Clear Pinecone and Re-embed All Semantic Layers

This script deletes ALL vectors from Pinecone and re-embeds all semantic layers.
Use this when semantic layer structure changes to avoid stale chunks.
"""
...
def main():
    """Main function to clear Pinecone and re-embed."""
    ...
    print("=" * 80)
    print("QueryDawg Pinecone Clear & Re-embed")
    print("=" * 80)
    print()
    ...
    # Get Pinecone index stats BEFORE
    print("\n📊 Pinecone Index Status BEFORE:")
    ...
    # Delete all vectors for each database
    print("=" * 80)
    print("🗑️  Deleting ALL existing vectors from Pinecone...")
    print("=" * 80)
    print()

    for layer_metadata in semantic_layers:
        database_name = layer_metadata['database_name']
        normalized_name = database_name.lower()

        print(f"Deleting vectors for: {database_name}")
        try:
    ...
    print("📊 Pinecone Index Status AFTER Deletion:")
    stats = embedding_service.get_index_stats()
    print(f"  Total vectors: {stats['total_vectors']:,}")
    print()

    if stats['total_vectors'] > 0:
        print("⚠️  Warning: Some vectors remain. Continuing anyway...")
        print()

    # Now re-embed everything
    print("=" * 80)
    print("🔄 Re-embedding All Semantic Layers")
    print("=" * 80)
    print()
    ...
    print("✅ All done! Fresh vectors are ready for semantic search.")
    print()
